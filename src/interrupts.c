...
static void BTN_Handler(void *InstancePtr){
	...
	switch(button_value){
		case 0: //Any Button Released
			...
			//want to check if the middle button is still being held
		if(button_value & MIDDLE_BUTTON_MASK) break;
			MIDDLE_BUTTON_PRESSED_FLAG = false;
			break;

		case MIDDLE_BUTTON_MASK: //1
			MIDDLE_BUTTON_PRESSED_FLAG = true;
			break;

		case DOWN_BUTTON_MASK: //2
			userInput.moveCursorVertically += 4;
			break;

		case LEFT_BUTTON_MASK: //4
			userInput.moveCursorHorizontally += -4;
			break;
